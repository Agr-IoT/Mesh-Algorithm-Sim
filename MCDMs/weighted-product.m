...
for i=1:n
    ...
    SN(i).score=-1; % score generated according to MCDM algorithm %
    SN(i).rank=-1; % rank assigned %
...
while operating_nodes>4
    ...
    % Node Ranking process %
    PRERANK=[];
    RANKED=[];
    AVG=0;
    if (rnd>0)
        for counterRow=1:n       % insert values into matrix
            PRERANK(counterRow,1) = SN(counterRow).id;
            PRERANK(counterRow,2) = SN(counterRow).E;
            PRERANK(counterRow,3) = SN(counterRow).dts;
            PRERANK(counterRow,4) = SN(counterRow).rop;
            PRERANK(counterRow,5) = SN(counterRow).dtch;
            PRERANK(counterRow,6) = SN(counterRow).role;
            
            %AVG=(SN(counterRow).E+SN(counterRow).dts+SN(counterRow).dtch)/3;
            %PRERANK(counterRow,6) = AVG; 
        end
        %RANKED = sortrows(PRERANK,5);
        
        MAX=max(PRERANK);
        MIN=min(PRERANK);
        
        BENEFICIAL_PROCESSED=zeros(size(PRERANK,1),1);
        for counterBeneficial=1:size(PRERANK,1) % runs counter to process beneficial attributes (Residual energy)
            BENEFICIAL_PROCESSED(counterBeneficial,1)=(PRERANK(counterBeneficial,2)/MAX(1,2))^0.6;
        endfor
        
        NONBENEFICIAL_PROCESSED=zeros(size(PRERANK,1),1);
        for counterNonBeneficial=1:size(PRERANK,1) % runs counter to process nonbeneficial attributes (Dist. to sink)
            NONBENEFICIAL_PROCESSED(counterNonBeneficial,1)=(MIN(1,3)/PRERANK(counterNonBeneficial,3))^0.4;
        endfor
        
        SCORE=zeros(size(PRERANK,1),2);
        rank=0;
        for counterScore=1:size(PRERANK,1)
            SN(counterScore).score=BENEFICIAL_PROCESSED(counterScore,1)*NONBENEFICIAL_PROCESSED(counterScore,1);
            SCORE(counterScore,1)=BENEFICIAL_PROCESSED(counterScore,1)*NONBENEFICIAL_PROCESSED(counterScore,1);
            rank=rank+1;
            SCORE(counterScore,2)=rank;
        endfor
